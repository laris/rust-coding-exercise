...
fn main() {
  ...
  // with a rang notation:
  if &str3[..] == magician1 {
  //if magician1 == &str3 {
    println!("We got the same magician alright too!")
  }
  ...
  // error: the trait `std::ops::Index<{integer}>` is not implemented for `str
  // println!("{}", magician1[1]);
  ...
  println!("{}", str5); // Ɵ Floor 1 is finished - Rise up to Floor 2

  // from_str:
  println!("{:?}", f64::from_str("3.6")); // Result { Ok(3.6) }
  let number: f64 = f64::from_str("3.6").unwrap();

  // string parameters:
  println!("Length of Merlin: {}", how_long(magician1));
  println!("Length of str1: {}", how_long(&str1));
  println!("Length of str1: {}", how_long(&str1[3..7]));

  // escape characters and raw string r"" or r#""#:
  println!("{}", "Ru\nst");
  println!("{}", r"Ru\nst");
  println!("{}", r#"#"Ru\nst"#);
}
