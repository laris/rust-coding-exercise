...
fn process_stream(mut stream: TcpStream) -> bool {
	...
	// prj 11 update new design
	let get1 = b"GET / HTTP/1.1\r\n";
	let get2 = b"GET /info HTTP/1.1\r\n";
	if buf.starts_with(get1) {
		println!("catch URL /");
	} else if buf.starts_with(get2) {
		let content = std::fs::read_to_string("info.html").unwrap();
		let content = content.replace("{id}", "001")
							.replace("{name}", "Alice")
							.replace("{gender}", "male");
		let len = content.len();
