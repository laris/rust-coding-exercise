...
fn add_bills(bills: &mut Vec<Bill>) {
    let bill = Bill::interactive_get_bill_from_input().unwrap();
    bills.push(bill);
}

fn remove_bill(bills: &mut Vec<Bill>) -> Result<(), Box<dyn std::error::Error>> {
    view_bills(bills);
    println!("Which bill do you want to remove? Input leading idx number:");
    let mut input = String::new();
    std::io::stdin().read_line(&mut input)?;
...
fn edit_bill(bills: &mut Vec<Bill>) -> Result<(), Box<dyn std::error::Error>>{
    view_bills(bills);
    println!("Which bill do you want to edit? Input leading idx number:");
    let mut input = String::new();
    std::io::stdin().read_line(&mut input)?; // read new input as String
    let idx: usize = input.trim().parse()?; // parse String to usize
    let new_bill = Bill::interactive_get_bill_from_input().unwrap(); // get new bill
    let old_bill = bills.remove(idx);
    println!("Are you confirm to replace \nthe old bill {old_bill:?}\nwith new bill {new_bill:?}? Yy/No?");
    input.clear();
    std::io::stdin().read_line(&mut input)?;
    let confirm = input.trim().to_lowercase();
    match confirm.as_str() {
        "y" | "yes" | "true" => { bills.insert(idx, new_bill); view_bills(bills); },
        "n" | "no" | "false" => { bills.insert(idx, old_bill); view_bills(bills); },
        _ => (),
    }
    Ok(())
}
