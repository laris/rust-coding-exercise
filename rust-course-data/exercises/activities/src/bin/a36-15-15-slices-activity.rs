...
fn main() {
    ...
    // empty data, not work because direct return None from iterator
    let mut stream = data_empty().chunks(2);
    println!("{:?}", stream);
    for chunk in stream {
        process_chunk(chunk);
    }
    ...
    for pairs in stream {
    match pairs {
        [a, b, c, ..] => println!("unreachable condidtion"),
        [first, second] => println!("Paired value: key={first:?}, value={second:?}"),
        [single] => println!("Unpaired value: {single:?}"),
        [] => println!("Data stream complete"),
    }
    }
